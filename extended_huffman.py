# https://www.youtube.com/watch?v=2LthKdnkR54&list=PLiqLGdpbmTW0vTp-V-5Rr9c3KzCvCg4HQ&index=14

# https://www.ques10.com/p/30204/explain-extended-huffman-coding-what-is-the-advant/


# the idea is that instead of encoding only one character instead what we do is we
# encode sequences of characters which in return might grant us with better results 
# better results meaning that we are closed to enropy than when we encode only one character you can refer to the article above
# for more examples
...
def create_encoding(tree,encoded_so_far,encoding_alphabet = {}):
    ...
    if len(tree.get_position().right.key) == 2:
    ...
    if len(tree.get_position().left.key) == 2 :
...
def main():
    ...
    # next we want to create sequences by concatanating the character and multiplying the probabilities


    sequence_probabilty = {}

    for i in range(len(ordered_characters)):
        for j in range(len(ordered_characters)):
            sequence_probabilty[ordered_characters[i]+ordered_characters[j]] = character_probability[ordered_characters[i]]*character_probability[ordered_characters[j]]
    ...
    for sequence,probabilty in sequence_probabilty.items():
        nodes.append(node(key = sequence,value= probabilty))
